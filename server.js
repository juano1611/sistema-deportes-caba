...
// GENERADOR DE PDF EXHAUSTIVO (CONTIENE EL 100% DE LOS CAMPOS DEL FORMULARIO)
        ...
        try {
            ...
            const mailOptions = {
                ...
                html: `
                    ...
                    <ul>
                        <li><strong>Título:</strong> ${p.titulo}</li>
                        <li><strong>Fecha:</strong> ${p.fecha}</li>
                        <li><strong>Lugar:</strong> ${p.lugar}</li>
                        <li><strong>Solicitante:</strong> ${p.responsableNombre} (DNI: ${p.responsableDni})</li>
                    </ul>
            ...
            console.log(`✉️ Email con PDF completo adjunto enviado a ${MAIL_DESTINO}`);
        } catch (mailErr) {
            console.error('⚠️ El pedido se guardó pero falló el envío de email:', mailErr);
        }

        return res.status(201).json({ msg: 'Pedido registrado y notificación enviada correctamente' });
    } catch (err) {
